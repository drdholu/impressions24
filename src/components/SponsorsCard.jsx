...
const SponsorsCard = ({ name, photo, description, link }) => {
  ...
  return (
    ...
    `}
    >
      <div className="relative w-80 h-60 border-2 rounded-lg shadow-lg bg-black overflow-hidden group transition-all duration-300 hover:scale-105 hover:shadow-2xl">
        {/* Image Section */}
        <div className="absolute inset-0 flex items-center justify-center transition-transform duration-500 transform group-hover:-translate-y-10"> {/* Move up on hover */}
          <img
            ...
            className="w-32 h-32 object-cover transition-transform duration-300 group-hover:scale-110" // Adjusted image size
          />
        </div>
        {/* Text Section */}
